<template>
  <div class="q-pa-md row items-start full-width full-height">
    <q-card
      class="q-mb-md full-width full-height"
      flat
      bordered
    >
      ...
      >
        ...
        <q-card-section class="row items-center justify-center">
          <q-circular-progress
            ...
            track-color="accent"
          >
            <div v-if="occupancyItem?.isValid"> {{ occupancyPercent }}% </div>
            <div v-else>
              <q-badge
                color="red"
                rounded
...
const imageUrl = computed(() =>
  props.item.id ? `/images/${props.item.img_url}` : 'https://cdn.quasar.dev/img/parallax2.jpg',
)
...
const { auditoryOccupancy } = useAuditoryOccupancy(refreshTrigger, props.url, {
