...
const { auditoriesInfo: roomsInfo, error: err } = useAuditoriesInfo(buildingIdRef, url, {
  optionalUrl: 'auditories',
  loading: true,
  notify: true,
})

const { auditoriesOccupancyInfo: auditoriesOccupancyInfo, error: errOccupancy } =
  useAuditoriesOccupancyInfo(buildingIdRef, url, {
  ...
  return roomsInfo.value
    .filter((room) => {
      ...
      return (
        ...
        room[locale.value].title
          .toLowerCase()
          .includes(filters.value.search.toString().toLowerCase())
      )
    })
    .filter((room) => {
      ...
      return (
        !filters.value.floor ||
        !filters.value.floor.length ||
        (Array.isArray(filters.value.floor) &&
          filters.value.floor.map((item) => item.value).includes(String(room.floor)))
      )
    })
    .filter((room) => {
      ...
      return (
        ...
        (Array.isArray(filters.value.type) &&
          filters.value.type.map((item) => item.value).includes(room.type))
      )
    })
})
</script>
