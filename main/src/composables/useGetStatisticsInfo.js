...
export function useStatisticsByDay(
  props,
  baseUrl,
  date,
  ...
  const queryParams = computed(() => {
    ...
    let str =
      dateRef.value.slice(-4) +
      '-' +
      dateRef.value.slice(-7, -5) +
      '-' +
      dateRef.value.slice(-10, -8)
    str = 'day=' + str + '&type=1'
  ...
  const { data, error, refetch } = useFetchList(props, baseUrl, {
    optionalUrl: url.value,
